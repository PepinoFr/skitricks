...
class ResetPasswordController extends AbstractController
{
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator): RedirectResponse
    {
        ...
        ;
        $mailer3 = new Swift_Mailer($transport);
        $message = (new Swift_Message('Wonderful Subject'))
                ->setFrom(['mathieulagnel@gmail.com' => 'mathieu lagnel'])
                ->setTo(['mathieulagnel@gmail.com', 'mathieulagnel@gmail.com' => 'toto '])
                ->setBody('lien pour la changer de mdp ' . $this->generateUrl('app_reset_password',array('token' =>$resetToken->getToken()),UrlGeneratorInterface::ABSOLUTE_URL)  )
            ;
        ;

// Send the message
        $result = $mailer3->send($message);

        $email = (new TemplatedEmail())
            ->from(new Address('mathieulagnel@gmail.com'))
